...
async function getCloudFiles(buttonEvent, files, setFiles) {
  ...
  const response = await fetch(`/api/getCloudFiles?client_name=${clean_name}`, {
    method: "GET",
    headers: {
        ...
        <div class={styles.modal}>
          ...
          <div className={styles.formItem}>
            ...
            <input
              ...
              value={clientName}
              onChange={(e) => {
                var clean_name = e.target.value.replace(/[^a-zA-Z0-9]/g, "");
                if (clean_name != e.target.value) {
                  setErrorMessage("Client Name must be alphanumeric");
                } else {
                  setErrorMessage("");
                }
            ...
            {files.map((file, index) => {
              return (
                <div class={styles.formItem} key={index}>
                  ...
                  />
                  <label htmlFor={file.slice(0, -4)}>{file.slice(0, -4)}</label>
                </div>
              );
            })}
          </div>
          <div className={styles.formItem}>
            <button
              ...
              onClick={async (e) => {
                ...
                var numUploaded = 0;
                var checkboxes = document.getElementsByName("cloudFiles");
